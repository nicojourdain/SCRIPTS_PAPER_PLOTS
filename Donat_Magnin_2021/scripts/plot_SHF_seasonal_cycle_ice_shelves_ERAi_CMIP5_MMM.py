...
file_pres='DATA/ERAi_ANx/MARv3.9.3_Amundsen_SHF_1979-2017_monthly.nc'
file_futu='DATA/ERAi_CMIP5anom_ANf/MARv3.9.3_Amundsen_SHF_2088-2117_monthly.nc'
...
shf=ncA['SHF'].values[108:468,:,:]  # only 1988-2017 to get the same interannual variability as future projection
shfmon=np.zeros((np.shape(shf)[0]))
for kmon in np.arange(np.shape(shf)[0]):
  shfmon[kmon]=np.nansum(np.nansum(shf[kmon,:,:]*msk,axis=1),axis=0)/np.nansum(np.nansum(msk,axis=1),axis=0)
climshfA=np.zeros((14))
tmp=np.zeros((12))
for kmon in np.arange(12):
  tmp[kmon] = np.mean(shfmon[kmon::12])
...
ax.plot(np.arange(14),climshfA,label='present (ERAinterim)',linewidth=1.0,color='cornflowerblue')
ax.plot(np.arange(14),climshfB,label='future (ERAinterim + CMIP5 anom)',linewidth=1.0,color='firebrick')
ax.set_xticks(np.arange(1,13,1))
ax.set_xticklabels(['M','J','J','A','S','O','N','D','J','F','M','A'])
plt.xlim(0.5,12.5)
ax.legend(fontsize=8)
ax.set_ylabel('Sensible heat flux (W.m-2)',size=12)
ax.tick_params(axis='both', which='major', labelsize=10)
#ax.text(1,75,'(a)',fontsize=18)

fig.savefig('SHF_seasonal_cycle_ice_shelves_ERAi_CMIP5.pdf')

diff_SHF=climshfB-climshfA
np.save('diff_SHF.npy',diff_SHF)
