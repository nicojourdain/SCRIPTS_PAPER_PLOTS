...
for kk in np.arange(0,np.size(xKDE)):
  kernel = np.exp(-((mltB+rnfB)/(snfB) - xKDE[kk]) ** 2 / (2 * Xsigma ** 2))
...
for kk in np.arange(np.size(xx)):
  for pp in np.arange(np.size(rofB)):
     if (mltB[pp]+rnfB[pp])/(snfB[pp]) > xx[kk]-0.5*dx and (mltB[pp]+rnfB[pp])/(snfB[pp]) <= xx[kk]+0.5*dx:
       if rofB[pp] >= thres:
         Nrof[kk]=Nrof[kk]+1
       else:
         Ndry[kk]=Ndry[kk]+1
...
print np.nanmin((mltB+rnfB)/(snfB))
print np.nanmax((mltB+rnfB)/(snfB))

#ax.scatter(mltA/snfA,rofA,s=5,c='cornflowerblue',label='Present')
ax.plot([0.0,130.0],[0.0,0.0],'-',color='k',linewidth=0.5)
ax.scatter((mltB+rnfB)/(snfB),rofB,s=5,c='red',alpha=0.2)
...
plt.ylim(-0.03,0.31)
#ax.legend()
ax.set_xlabel('(melt+rainfall)/snowfall',size=12)
...
#plt.show()
fig.savefig('runoff_vs_melt_on_snf_ratio_rain_bis.pdf')
