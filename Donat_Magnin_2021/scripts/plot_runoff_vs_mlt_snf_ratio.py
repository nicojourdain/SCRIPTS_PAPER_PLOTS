...
xKDE=np.arange(0,4,0.01)
Xsigma=0.1

fit=np.zeros(np.shape(xKDE))

for kk in np.arange(0,np.size(xKDE)):
  kernel = np.exp(-(mltB/snfB - xKDE[kk]) ** 2 / (2 * Xsigma ** 2))
...
print np.shape(Nrof), np.shape(xx)
for kk in np.arange(np.size(xx)):
  for pp in np.arange(np.size(rofB)):
     if mltB[pp]/snfB[pp] > xx[kk]-0.5*dx and mltB[pp]/snfB[pp] <= xx[kk]+0.5*dx:
       if rofB[pp] >= thres:
         Nrof[kk]=Nrof[kk]+1
       else:
         Ndry[kk]=Ndry[kk]+1
...
ax.plot([0.0,2.0],[0.0,0.0],'-',color='k',linewidth=0.5)
ax.scatter(mltB/snfB,rofB,s=5,c='red',alpha=0.2)
...
plt.ylim(-0.03,0.31)
#ax.legend()
ax.set_xlabel('melt/snowfall',size=12)
ax.set_ylabel('Net Liquid (m.w.e/yr)',size=12)
ax.text(0.32,0.28,'10%',color='firebrick',fontsize=10)
ax.text(0.87,0.28,'50%',color='firebrick',fontsize=10)

#plt.show()
fig.savefig('runoff_vs_melt_on_snf_ratio.pdf')
