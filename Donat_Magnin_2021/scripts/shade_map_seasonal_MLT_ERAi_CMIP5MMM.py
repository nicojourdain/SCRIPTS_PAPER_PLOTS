...
file_mlt_pres=pref_pres+'mlt'+suff_pres
ncSp=xr.open_dataset(file_mlt_pres,decode_cf=False)
mlt_JAN_p=np.nansum(ncSp['mlt'].values[9*12+0::12,0,j1:j2,i1:i2],axis=0)*msk  # in mm.w.e/month
mlt_FEB_p=np.nansum(ncSp['mlt'].values[9*12+1::12,0,j1:j2,i1:i2],axis=0)*msk  # in mm.w.e/month
mlt_MAR_p=np.nansum(ncSp['mlt'].values[9*12+2::12,0,j1:j2,i1:i2],axis=0)*msk  # in mm.w.e/month
mlt_APR_p=np.nansum(ncSp['mlt'].values[9*12+3::12,0,j1:j2,i1:i2],axis=0)*msk  # in mm.w.e/month
mlt_MAY_p=np.nansum(ncSp['mlt'].values[9*12+4::12,0,j1:j2,i1:i2],axis=0)*msk  # in mm.w.e/month
mlt_JUN_p=np.nansum(ncSp['mlt'].values[9*12+5::12,0,j1:j2,i1:i2],axis=0)*msk  # in mm.w.e/month
mlt_JUL_p=np.nansum(ncSp['mlt'].values[9*12+6::12,0,j1:j2,i1:i2],axis=0)*msk  # in mm.w.e/month
mlt_AUG_p=np.nansum(ncSp['mlt'].values[9*12+7::12,0,j1:j2,i1:i2],axis=0)*msk  # in mm.w.e/month
mlt_SEP_p=np.nansum(ncSp['mlt'].values[9*12+8::12,0,j1:j2,i1:i2],axis=0)*msk  # in mm.w.e/month
mlt_OCT_p=np.nansum(ncSp['mlt'].values[9*12+9::12,0,j1:j2,i1:i2],axis=0)*msk  # in mm.w.e/month
mlt_NOV_p=np.nansum(ncSp['mlt'].values[9*12+10::12,0,j1:j2,i1:i2],axis=0)*msk # in mm.w.e/month
mlt_DEC_p=np.nansum(ncSp['mlt'].values[9*12+11::12,0,j1:j2,i1:i2],axis=0)*msk # in mm.w.e/month

file_mlt_futu=pref_futu+'mlt'+suff_futu
ncSf=xr.open_dataset(file_mlt_futu,decode_cf=False)
mlt_JAN_f=np.nansum(ncSf['mlt'].values[0::12,0,j1:j2,i1:i2],axis=0)*msk  # in mm.w.e/month
mlt_FEB_f=np.nansum(ncSf['mlt'].values[1::12,0,j1:j2,i1:i2],axis=0)*msk  # in mm.w.e/month
mlt_MAR_f=np.nansum(ncSf['mlt'].values[2::12,0,j1:j2,i1:i2],axis=0)*msk  # in mm.w.e/month
mlt_APR_f=np.nansum(ncSf['mlt'].values[3::12,0,j1:j2,i1:i2],axis=0)*msk  # in mm.w.e/month
mlt_MAY_f=np.nansum(ncSf['mlt'].values[4::12,0,j1:j2,i1:i2],axis=0)*msk  # in mm.w.e/month
mlt_JUN_f=np.nansum(ncSf['mlt'].values[5::12,0,j1:j2,i1:i2],axis=0)*msk  # in mm.w.e/month
mlt_JUL_f=np.nansum(ncSf['mlt'].values[6::12,0,j1:j2,i1:i2],axis=0)*msk  # in mm.w.e/month
mlt_AUG_f=np.nansum(ncSf['mlt'].values[7::12,0,j1:j2,i1:i2],axis=0)*msk  # in mm.w.e/month
mlt_SEP_f=np.nansum(ncSf['mlt'].values[8::12,0,j1:j2,i1:i2],axis=0)*msk  # in mm.w.e/month
mlt_OCT_f=np.nansum(ncSf['mlt'].values[9::12,0,j1:j2,i1:i2],axis=0)*msk  # in mm.w.e/month
mlt_NOV_f=np.nansum(ncSf['mlt'].values[10::12,0,j1:j2,i1:i2],axis=0)*msk # in mm.w.e/month
mlt_DEC_f=np.nansum(ncSf['mlt'].values[11::12,0,j1:j2,i1:i2],axis=0)*msk # in mm.w.e/month

mlt_anom_SON = mlt_SEP_f + mlt_OCT_f + mlt_NOV_f - ( mlt_SEP_p + mlt_OCT_p + mlt_NOV_p )  # in mm.w.e/(3 months)
mlt_anom_DJF = mlt_DEC_f + mlt_JAN_f + mlt_FEB_f - ( mlt_DEC_p + mlt_JAN_p + mlt_FEB_p )
mlt_anom_MAM = mlt_MAR_f + mlt_APR_f + mlt_MAY_f - ( mlt_MAR_p + mlt_APR_p + mlt_MAY_p )
mlt_anom_JJA = mlt_JUN_f + mlt_JUL_f + mlt_AUG_f - ( mlt_JUN_p + mlt_JUL_p + mlt_AUG_p )
...
Ncool=np.int(256*10./(100.+10.))
Nwarm=256-Ncool
col = cm.get_cmap('PuOr', 256)
tmp1 = col(np.linspace(0.35, 1.00, Ncool)) # decrease first number to have more white in the middle light-blue colors
...
im0=axs[0].contourf(x2d,y2d,mlt_anom_SON,np.arange(-5.0,52.5,2.5),cmap=cmap_new,extend='both')
...
cbar0=fig.colorbar(im0, cax=axins0, orientation="horizontal", ticks=np.arange(0.0,60.0,10.0))
cbar0.ax.tick_params(labelsize=8)
axins0.xaxis.set_ticks_position("bottom")
axs[0].set_title('(a) SON (mm w. e. (3 months)$^{-1}$)',fontsize=8)

im1=axs[1].contourf(x2d,y2d,mlt_anom_DJF,np.arange(-50.0,525.0,25.0),cmap=cmap_new,extend='both')
...
cbar1=fig.colorbar(im1, cax=axins1, orientation="horizontal", ticks=np.arange(0.0,600.0,100.0))
cbar1.ax.tick_params(labelsize=8,color='red',labelcolor='red')
axins1.xaxis.set_ticks_position("bottom")
axs[1].set_title('(b) DJF (mm w. e. (3 months)$^{-1}$)',fontsize=8)

im2=axs[2].contourf(x2d,y2d,mlt_anom_MAM,np.arange(-5.0,52.5,2.5),cmap=cmap_new,extend='both')
...
cbar2=fig.colorbar(im2, cax=axins2, orientation="horizontal", ticks=np.arange(0.0,60.0,10.0))
cbar2.ax.tick_params(labelsize=8)
axins2.xaxis.set_ticks_position("bottom")
axs[2].set_title('(c) MAM (mm w. e. (3 months)$^{-1}$)',fontsize=8)

im3=axs[3].contourf(x2d,y2d,mlt_anom_JJA,np.arange(-5.0,52.5,2.5),cmap=cmap_new,extend='both')
...
cbar3=fig.colorbar(im3, cax=axins3, orientation="horizontal", ticks=np.arange(0.0,60.0,10.0))
cbar3.ax.tick_params(labelsize=8)
axins3.xaxis.set_ticks_position("bottom")
axs[3].set_title('(d) JJA (mm w. e. (3 months)$^{-1}$)',fontsize=8)

fig.savefig('map_seasonal_MLT_ERAi_CMIP5MMM.pdf')
