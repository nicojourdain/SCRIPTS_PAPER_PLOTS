...
file_pres='DATA/ERAi_ANx/MARv3.9.3_Amundsen_ALSN_1979-2017_monthly.nc'
file_futu='DATA/ERAi_CMIP5anom_ANf/MARv3.9.3_Amundsen_ALSN_2088-2117_monthly.nc'
...
alsn=ncA['ALSN'].values[108:468,:,:]  # only 1988-2017 to get the same interannual variability as future projection
alsnmon=np.zeros((np.shape(alsn)[0]))
for kmon in np.arange(np.shape(alsn)[0]):
  alsnmon[kmon]=np.nansum(np.nansum(alsn[kmon,:,:]*msk,axis=1),axis=0)/np.nansum(np.nansum(msk,axis=1),axis=0)
climalsnA=np.zeros((14))
tmp=np.zeros((12))
for kmon in np.arange(12):
  tmp[kmon] = np.mean(alsnmon[kmon::12])
...
alsn=ncB['ALSN'].values[:,:,:]
alsnmon=np.zeros((np.shape(alsn)[0]))
for kmon in np.arange(np.shape(alsn)[0]):
  alsnmon[kmon]=np.nansum(np.nansum(alsn[kmon,:,:]*msk,axis=1),axis=0)/np.nansum(np.nansum(msk,axis=1),axis=0)
climalsnB=np.zeros((14))
tmp=np.zeros((12))
for kmon in np.arange(12):
  tmp[kmon] = np.mean(alsnmon[kmon::12])
tmp=np.roll(tmp,-4) # to start in May instead of January
...
ax.plot(np.arange(14),climalsnA,label='present (ERAinterim)',linewidth=1.0,color='cornflowerblue')
ax.plot(np.arange(14),climalsnB,label='future (ERAinterim + CMIP5 anom)',linewidth=1.0,color='firebrick')
ax.set_xticks(np.arange(1,13,1))
ax.set_xticklabels(['M','J','J','A','S','O','N','D','J','F','M','A'])
plt.xlim(0.5,12.5)
ax.legend(fontsize=8)
ax.set_ylabel('Albedo (-)',size=12)
ax.tick_params(axis='both', which='major', labelsize=10)
#ax.text(1,75,'(a)',fontsize=18)

fig.savefig('ALSN_seasonal_cycle_ice_shelves_ERAi_CMIP5.pdf')
