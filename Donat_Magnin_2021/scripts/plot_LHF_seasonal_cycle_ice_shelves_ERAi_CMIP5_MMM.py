...
file_pres='DATA/ERAi_ANx/MARv3.9.3_Amundsen_LHF_1979-2017_monthly.nc'
file_futu='DATA/ERAi_CMIP5anom_ANf/MARv3.9.3_Amundsen_LHF_2088-2117_monthly.nc'
...
lhf=ncA['LHF'].values[108:468,:,:]  # only 1988-2017 to get the same interannual variability as future projection
lhfmon=np.zeros((np.shape(lhf)[0]))
for kmon in np.arange(np.shape(lhf)[0]):
  lhfmon[kmon]=np.nansum(np.nansum(lhf[kmon,:,:]*msk,axis=1),axis=0)/np.nansum(np.nansum(msk,axis=1),axis=0)
climlhfA=np.zeros((14))
tmp=np.zeros((12))
for kmon in np.arange(12):
  tmp[kmon] = np.mean(lhfmon[kmon::12])
...
ax.plot(np.arange(14),climlhfA,label='present (ERAinterim)',linewidth=1.0,color='cornflowerblue')
ax.plot(np.arange(14),climlhfB,label='future (ERAinterim + CMIP5 anom)',linewidth=1.0,color='firebrick')
ax.set_xticks(np.arange(1,13,1))
ax.set_xticklabels(['M','J','J','A','S','O','N','D','J','F','M','A'])
plt.xlim(0.5,12.5)
ax.legend(fontsize=8)
ax.set_ylabel('Latent heat flux (W.m-2)',size=12)
ax.tick_params(axis='both', which='major', labelsize=10)
#ax.text(1,75,'(a)',fontsize=18)

fig.savefig('LHF_seasonal_cycle_ice_shelves_ERAi_CMIP5.pdf')

diff_LHF=climlhfB-climlhfA
np.save('diff_LHF.npy',diff_LHF)
