...
for kmon in np.arange(np.shape(rof)[0]):
  rofmon_ABB[kmon]=np.nansum(np.nansum(rof[kmon,:,:]*msk*msk_ABB*(1-msk_isf_ABB),axis=1),axis=0)*365.25*1.e-12*1.e8 # Gt/yr
  rofmon_COS[kmon]=np.nansum(np.nansum(rof[kmon,:,:]*msk*msk_COS*(1-msk_isf_COS),axis=1),axis=0)*365.25*1.e-12*1.e8 # Gt/yr
  rofmon_PIG[kmon]=np.nansum(np.nansum(rof[kmon,:,:]*msk*msk_PIG*(1-msk_isf_PIG),axis=1),axis=0)*365.25*1.e-12*1.e8 # Gt/yr
  rofmon_THW[kmon]=np.nansum(np.nansum(rof[kmon,:,:]*msk*msk_THW*(1-msk_isf_THW),axis=1),axis=0)*365.25*1.e-12*1.e8 # Gt/yr
  rofmon_CRO[kmon]=np.nansum(np.nansum(rof[kmon,:,:]*msk*msk_CRO*(1-msk_isf_CRO),axis=1),axis=0)*365.25*1.e-12*1.e8 # Gt/yr
  rofmon_DOT[kmon]=np.nansum(np.nansum(rof[kmon,:,:]*msk*msk_DOT*(1-msk_isf_DOT),axis=1),axis=0)*365.25*1.e-12*1.e8 # Gt/yr
  rofmon_GET[kmon]=np.nansum(np.nansum(rof[kmon,:,:]*msk*msk_GET*(1-msk_isf_GET),axis=1),axis=0)*365.25*1.e-12*1.e8 # Gt/yr
print 'Runoff        &       ',np.round(np.mean(rofmon_ABB),1),'  &       ',\
                               np.round(np.mean(rofmon_COS),1),'  &       ',\
                               np.round(np.mean(rofmon_PIG),1),'  &       ',\
                               np.round(np.mean(rofmon_THW),1),'  &       ',\
                               np.round(np.mean(rofmon_CRO),1),'  &       ',\
                               np.round(np.mean(rofmon_DOT),1),'  &       ',\
                               np.round(np.mean(rofmon_GET),1),' \\\\'
...
for kmon in np.arange(np.shape(rof)[0]):
  ...
  rofmon_DOT[kmon]=np.nansum(np.nansum(rof[kmon,:,:]*msk*msk_DOT*(1-msk_isf_DOT),axis=1),axis=0)*365.25*1.e-12*1.e8 # Gt/yr
  rofmon_GET[kmon]=np.nansum(np.nansum(rof[kmon,:,:]*msk*msk_GET*(1-msk_isf_GET),axis=1),axis=0)*365.25*1.e-12*1.e8 # Gt/yr
print '              &  {\\bf ',np.round(np.mean(rofmon_ABB),1),'} & {\\bf ',\
                                np.round(np.mean(rofmon_COS),1),'} & {\\bf ',\
                                np.round(np.mean(rofmon_PIG),1),'} & {\\bf ',\
                                np.round(np.mean(rofmon_THW),1),'} & {\\bf ',\
                                np.round(np.mean(rofmon_CRO),1),'} & {\\bf ',\
                                np.round(np.mean(rofmon_DOT),1),'} & {\\bf ',\
                                np.round(np.mean(rofmon_GET),1),'} \\\\'
