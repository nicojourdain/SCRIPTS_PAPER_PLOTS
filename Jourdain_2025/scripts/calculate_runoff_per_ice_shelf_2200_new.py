...
filez_runoff_dates='runoff_dates_2200_new.npz'
...
if not os.path.exists(filez_runoff_dates):
   ...
   avgru_isf_ens_ssp126 = np.zeros((Ntime,Nmod*Nwt,Nbasin)) * np.nan
   avgru_isf_ens_ssp585 = np.zeros((Ntime,Nmod*Nwt,Nbasin)) * np.nan

   avgru_bas_ens_ssp126 = np.zeros((Ntime,Nmod*Nwt,Nbasin)) * np.nan
   avgru_bas_ens_ssp585 = np.zeros((Ntime,Nmod*Nwt,Nbasin)) * np.nan
   ...
   for kmod in np.arange(Nmod):
      ...
      for kbasin in np.arange(Nbasin):
         ...
         if (   ( ( ( area_isf*4.0**2 > 1500 ) & ( area_grd*4.0**2 > 15000 ) ) \
           ...
           tmp_bas = ((d0.ru+d3.aru) * ( ratio[kbasin]*mskbas + (1-ratio[kbasin])*mskisf ) ).sum(dim=["x","y"]) * 365.25 * 86400 / area_isf # assuming that a fraction of the runoff produced in this basin flows to the ice shelf [kg m-2 yr-1]
           for kkw in np.arange(Nwt):
             avgru_isf_ens_ssp126[:,kmod*Nwt+kkw,kbasin] = tmp_isf - threshold[kkw]
             avgru_bas_ens_ssp126[:,kmod*Nwt+kkw,kbasin] = tmp_bas - threshold[kkw]
           # SSP585:
           ...
           for kkw in np.arange(Nwt):
             avgru_isf_ens_ssp585[:,kmod*Nwt+kkw,kbasin] = tmp_isf - threshold[kkw]
             avgru_bas_ens_ssp585[:,kmod*Nwt+kkw,kbasin] = tmp_bas - threshold[kkw]
  
   #-- Smoothing and pseudo-percentiles (all models for 90% ; 5/8 models for 66% range) across models: 
   ...
   ##-- Smoothing and pseudo-percentiles (all models for 90% ; 5/8 models for 66% range) across models: 
   #avgru_isf_ssp126_pct05 = np.nanmin(moving_average(avgru_isf_ens_ssp126, timewin),axis=1)
   #avgru_isf_ssp126_pct17 = np.nanpercentile(moving_average(avgru_isf_ens_ssp126, timewin), 7.14,axis=1) # =100*0.5/(8-1)  (between 1st and 2nd model)
   #avgru_isf_ssp126_pct83 = np.nanpercentile(moving_average(avgru_isf_ens_ssp126, timewin),78.57,axis=1) # =100*5.5/(8-1)  (between 6th and 7th model)
   #avgru_isf_ssp126_pct95 = np.nanmax(moving_average(avgru_isf_ens_ssp126, timewin),axis=1)

   #avgru_isf_ssp585_pct05 = np.nanmin(moving_average(avgru_isf_ens_ssp585, timewin),axis=1)
   #avgru_isf_ssp585_pct17 = np.nanpercentile(moving_average(avgru_isf_ens_ssp585, timewin), 7.14,axis=1) # =100*0.5/(8-1)  (between 1st and 2nd model)
   #avgru_isf_ssp585_pct83 = np.nanpercentile(moving_average(avgru_isf_ens_ssp585, timewin),78.57,axis=1) # =100*5.5/(8-1)  (between 6th and 7th model)
   #avgru_isf_ssp585_pct95 = np.nanmax(moving_average(avgru_isf_ens_ssp585, timewin),axis=1)

   #avgru_bas_ssp126_pct05 = np.nanmin(moving_average(avgru_bas_ens_ssp126, timewin),axis=1)
   #avgru_bas_ssp126_pct17 = np.nanpercentile(moving_average(avgru_bas_ens_ssp126, timewin), 7.14,axis=1) # =100*0.5/(8-1)  (between 1st and 2nd model)
   #avgru_bas_ssp126_pct83 = np.nanpercentile(moving_average(avgru_bas_ens_ssp126, timewin),78.57,axis=1) # =100*5.5/(8-1)  (between 6th and 7th model)
   #avgru_bas_ssp126_pct95 = np.nanmax(moving_average(avgru_bas_ens_ssp126, timewin),axis=1)

   #avgru_bas_ssp585_pct05 = np.nanmin(moving_average(avgru_bas_ens_ssp585, timewin),axis=1)
   #avgru_bas_ssp585_pct17 = np.nanpercentile(moving_average(avgru_bas_ens_ssp585, timewin), 7.14,axis=1) # =100*0.5/(8-1)  (between 1st and 2nd model)
   #avgru_bas_ssp585_pct83 = np.nanpercentile(moving_average(avgru_bas_ens_ssp585, timewin),78.57,axis=1) # =100*5.5/(8-1)  (between 6th and 7th model)
   #avgru_bas_ssp585_pct95 = np.nanmax(moving_average(avgru_bas_ens_ssp585, timewin),axis=1)
