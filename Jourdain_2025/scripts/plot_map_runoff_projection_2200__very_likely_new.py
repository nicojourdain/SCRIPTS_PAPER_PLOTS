...
for kbasin in np.arange(Nbasin):
  ...
  if ( not np.isnan(xboce[kbasin]) ):
    ...
    if (  ( basin.NAME[kbasin].values == 'Nordenskjöld, Marin, Harbord,\nCheetham, Geikie Inlet' ) \
      ...
      ddyy = 0.e0
    #--
    if ( (year_bas_ssp126_pct95[kbasin] == 1500) & (year_bas_ssp126_pct05[kbasin] == 1500) ):
      textssp126='always'
    elif ( (year_bas_ssp126_pct95[kbasin] == 2500) & (year_bas_ssp126_pct05[kbasin] == 2500) ):
      textssp126='never'
    elif ( (year_bas_ssp126_pct95[kbasin] == 1500) & (year_bas_ssp126_pct05[kbasin] == 2500) ):
      textssp126='alw.-nev.'
    elif (year_bas_ssp126_pct95[kbasin] == 1500):
      textssp126='alw.-'+year_bas_ssp126_pct05[kbasin].astype('str')
    elif (year_bas_ssp126_pct05[kbasin] == 2500):
      textssp126=year_bas_ssp126_pct95[kbasin].astype('str')+'-nev.'
    else:
      textssp126=year_bas_ssp126_pct95[kbasin].astype('str')+'-'+year_bas_ssp126_pct05[kbasin].astype('str')
    #--
    if ( (year_bas_ssp585_pct95[kbasin] == 1500) & (year_bas_ssp585_pct05[kbasin] == 1500) ):
      textssp585='always'
    elif ( (year_bas_ssp585_pct95[kbasin] == 2500) & (year_bas_ssp585_pct05[kbasin] == 2500) ):
      textssp585='never'
    elif ( (year_bas_ssp585_pct95[kbasin] == 1500) & (year_bas_ssp585_pct05[kbasin] == 2500) ):
      textssp585='alw.-nev.'
    elif (year_bas_ssp585_pct95[kbasin] == 1500):
      textssp585='alw.-'+year_bas_ssp585_pct05[kbasin].astype('str')
    elif (year_bas_ssp585_pct05[kbasin] == 2500):
      textssp585=year_bas_ssp585_pct95[kbasin].astype('str')+'-nev.'
    else:
      textssp585=year_bas_ssp585_pct95[kbasin].astype('str')+'-'+year_bas_ssp585_pct05[kbasin].astype('str')
    #--
    if (year_bas_ssp126_pct05[kbasin] <= 2014):
       coltex = 'darkred' # Full likely range before 2015
    elif ( (year_bas_ssp126_pct95[kbasin] <= 2014) | (year_bas_ssp585_pct95[kbasin] <= 2014) ):
       coltex = 'red'     # Likely range starting before 2015
    elif (year_bas_ssp585_pct95[kbasin] <= 2050):
       coltex = 'orchid'  # likely range starting before 2050 in at least one SSP
    elif (year_bas_ssp585_pct95[kbasin] <= 2100):
...
axs.text(-2.80e6,-2.4e6,'Full very likely range before 2015',color='darkred',fontsize=13,fontweight='bold')
axs.text(-2.80e6,-2.5e6,'Very likely range starting before 2015',color='red',fontsize=13,fontweight='bold')
axs.text(-2.80e6,-2.6e6,'Very likely range starting before 2050 in at least one SSP',color='orchid',fontsize=13,fontweight='bold')
axs.text(-2.80e6,-2.7e6,'Very likely range starting before 2100 in SSP5-8.5',color='blueviolet',fontsize=13,fontweight='bold')
axs.text(-2.80e6,-2.8e6,'Very likely range starting after 2100 in SSP5-8.5',color='darkblue',fontsize=13,fontweight='bold')

# Additional panel on the evolution of number of ice shelves above the threshold :
yyyy=np.arange(1850,2201,1)
Ny=np.size(yyyy)
nb_isf_ssp126_pct95 = np.zeros((Ny))
nb_isf_ssp126_pct05 = np.zeros((Ny))
nb_isf_ssp585_pct95 = np.zeros((Ny))
nb_isf_ssp585_pct05 = np.zeros((Ny))
for ky in np.arange(Ny):
   nb_isf_ssp126_pct95[ky] = np.sum( year_bas_ssp126_pct95 <= yyyy[ky] )
   nb_isf_ssp126_pct05[ky] = np.sum( year_bas_ssp126_pct05 <= yyyy[ky] )
   nb_isf_ssp585_pct95[ky] = np.sum( year_bas_ssp585_pct95 <= yyyy[ky] )
   nb_isf_ssp585_pct05[ky] = np.sum( year_bas_ssp585_pct05 <= yyyy[ky] )
delta = nb_isf_ssp126_pct95[0]
nb_isf_ssp126_pct95 = nb_isf_ssp126_pct95 - delta + 14
nb_isf_ssp126_pct05 = nb_isf_ssp126_pct05 - delta + 14
nb_isf_ssp585_pct95 = nb_isf_ssp585_pct95 - delta + 14
nb_isf_ssp585_pct05 = nb_isf_ssp585_pct05 - delta + 14
llax=fig.add_axes([0.17, 0.02, 0.32, 0.10])
llax.fill_between(yyyy,nb_isf_ssp126_pct95,nb_isf_ssp126_pct05,color='cornflowerblue',alpha=0.2)
llax.fill_between(yyyy,nb_isf_ssp585_pct95,nb_isf_ssp585_pct05,color='darkred',alpha=0.2)
...
llax.set_ylim([0,np.max(nb_isf_ssp585_pct05)])

xc=np.mean(axs.get_xlim())
yu=axs.get_ylim()[1]
axs.text(xc,0.98*yu,'Very likely emergence of surface conditions necessary for hydrofracturing',fontsize=20,fontweight='bold',ha='center')
axs.set_axis_off()

##########################################################################

fig.savefig("map_runoff_projection_very_likely_range_new.pdf")
