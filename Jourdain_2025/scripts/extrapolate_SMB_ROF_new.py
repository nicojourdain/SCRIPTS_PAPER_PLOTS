...
CMIPmod = 'IPSL-CM6A-LR'  # 'IPSL-CM6A-LR', 'CNRM-CM6-1', 'MPI-ESM1-2-HR' , 'UKESM1-0-LL', 'CESM2'
                         # 'ACCESS1-3' , 'NorESM1-M'
...
if scat:
  ...
  cax0=axs[0].contourf(TT,RR,np.transpose(PDF_SMB),20,cmap='YlOrBr')
...
axs[0].plot(TT,Yf,linewidth=1.5,color='k',label=r'fit : exp('+string+' $\Delta$T)')
...
if scat:
  ...
  cax1=axs[1].contourf(XX,MM,np.transpose(PDF_MLT),20,cmap='YlOrBr')
...
axs[1].plot(XX,Zf,linewidth=1.5,color='k',label=r'fit : exp('+string+' $\Delta$T)')
