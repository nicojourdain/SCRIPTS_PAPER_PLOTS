...
file_rnf_p='/Users/njourdain/OUTPUT_AMU/runoff_climato_AMUXL12_GNJ002_1989-2009.nc'
file_rnf_f='/Users/njourdain/OUTPUT_AMU/runoff_climato_AMUXL12_GNJ002_2080-2100.nc'

ncp = xr.open_dataset(file_rnf_p,decode_cf=False)
ncf = xr.open_dataset(file_rnf_f,decode_cf=False)

print(ncp.runoff.shape)

rnf_p = ncp.runoff.mean(axis=0)
rnf_f = ncf.runoff.mean(axis=0)
...
tmp_rnf_p = rnf_p * msk_shelf *  ncmsh.e1t.isel(t=0) * ncmsh.e2t.isel(t=0)
tmp_rnf_f = rnf_f * msk_shelf *  ncmsh.e1t.isel(t=0) * ncmsh.e2t.isel(t=0)

total_rnf_p = tmp_rnf_p.sum() * 86400 * 365.25 * 1.e-12 # Gt/yr
total_rnf_f = tmp_rnf_f.sum() * 86400 * 365.25 * 1.e-12 # Gt/yr
...
cax0=axs[0].pcolormesh(lon2d, lat2d, rnf_p*86400, vmin=0., vmax=4., rasterized=True, cmap='magma', transform=trans )
...
axs[0].set_title('(g) Iceberg melt (1989-2009)',size=14,fontweight='bold')
...
string=np.array2string(total_rnf_p.values,precision=1,floatmode='fixed')
...
cax1=axs[1].pcolormesh(lon2d, lat2d, (rnf_f-rnf_p)*86400, vmin=-6., vmax=6., rasterized=True, cmap='RdBu_r', transform=trans )
...
axs[1].set_title('(h) Iceberg melt anomaly',size=14,fontweight='bold')

#colorbar:
cbar1=fig.colorbar(cax1,ax=axs[1],fraction=0.029, pad=0.02, extend='both', ticks=np.arange(-6,7.,1.0))
...
# Plot mean value over the Amundsen continental shelf:
diff = (total_rnf_f-total_rnf_p).values
...
#--------------------
#fig.tight_layout()
fig.savefig('map_runoff_anomaly.jpg') # warning: do not specify dpi
fig.savefig('map_runoff_anomaly.pdf')
#plt.show()
