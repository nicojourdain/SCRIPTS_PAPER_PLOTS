...
if not os.path.isfile('TS_profiles_ContShelf_noisf.npz'):
  ...
  np.savez('TS_profiles_ContShelf_noisf.npz',mean_Tp=mean_Tp,mean_Tf=mean_Tf,mean_Sp=mean_Sp,mean_Sf=mean_Sf,ZZ=ZZ)

else:

  print('  ')
  print('WARNING !!! USING EXISTING TS_profiles_ContShelf_noisf.npz !!!')
  print('  Delete before running this script if you want to regenerate it.')
  print('  ')

  npzfile = np.load('TS_profiles_ContShelf_noisf.npz')
...
fig.savefig('TS_profiles_ContShelf_noisf.jpg')
fig.savefig('TS_profiles_ContShelf_noisf.pdf')
