...
print(ncpA.sithic.shape)

# Average over A, B, C simulations :
print(ncpA.sithic.isel(time_counter=slice(7,10)).shape)
sithic_ASO_p = ( ncpA.sithic.isel(time_counter=slice(7,10)).mean(axis=0) + ncpB.sithic.isel(time_counter=slice(7,10)).mean(axis=0) + ncpC.sithic.isel(time_counter=slice(7,10)).mean(axis=0) ) /3.
sithic_ASO_f = ( ncfA.sithic.isel(time_counter=slice(7,10)).mean(axis=0) + ncfB.sithic.isel(time_counter=slice(7,10)).mean(axis=0) + ncfC.sithic.isel(time_counter=slice(7,10)).mean(axis=0) ) /3.
...
tmp_sithic_ASO_p = sithic_ASO_p * msk_shelf *  ncmsh.e1t.isel(t=0) * ncmsh.e2t.isel(t=0)
tmp_sithic_ASO_f = sithic_ASO_f * msk_shelf *  ncmsh.e1t.isel(t=0) * ncmsh.e2t.isel(t=0)
tmp_area = msk_shelf *  ncmsh.e1t.isel(t=0) * ncmsh.e2t.isel(t=0)

mean_sithic_ASO_p = tmp_sithic_ASO_p.sum() / tmp_area.sum()
mean_sithic_ASO_f = tmp_sithic_ASO_f.sum() / tmp_area.sum()
...
cax0=axs[0].pcolormesh(lon2d, lat2d, sithic_ASO_p, vmin=0., vmax=3., rasterized=True, cmap='magma', transform=trans )
...
axs[0].set_title('(e) ASO sea ice thickness (1989-2009)',size=14,fontweight='bold')

#colorbar:
cbar0=fig.colorbar(cax0,ax=axs[0],fraction=0.029, pad=0.02, extend='max', ticks=np.arange(0,3.3,0.3))
cbar0.ax.set_title('m',size=12)
...
string=np.array2string(mean_sithic_ASO_p.values,precision=2,floatmode='fixed')
axs[0].text(xmin+0.365*(xmax-xmin), ymin+0.20*(ymax-ymin),string,color='black',
            fontsize=14,fontweight='bold',ha='right')
axs[0].text(xmin+0.370*(xmax-xmin), ymin+0.20*(ymax-ymin),'m',color='black',fontsize=14,fontweight='bold')
...
cax1=axs[1].pcolormesh(lon2d, lat2d, sithic_ASO_f, vmin=0., vmax=3., rasterized=True, cmap='magma', transform=trans )
...
axs[1].set_title('(f) ASO sea ice thickness (2080-2100)',size=14,fontweight='bold')

#colorbar:
cbar1=fig.colorbar(cax1,ax=axs[1],fraction=0.029, pad=0.02, extend='max', ticks=np.arange(0,3.3,0.3))
cbar1.ax.set_title('m',size=12)
...
# Plot mean value over the Amundsen continental shelf:
string=np.array2string(mean_sithic_ASO_f.values,precision=2,floatmode='fixed')
axs[1].text(xmin+0.365*(xmax-xmin), ymin+0.20*(ymax-ymin),string,color='k',
            fontsize=14,fontweight='bold',ha='right')
axs[1].text(xmin+0.370*(xmax-xmin), ymin+0.20*(ymax-ymin),'m',color='black',fontsize=14,fontweight='bold')

#--------------------
#fig.tight_layout()
fig.savefig('map_sithic_ASO_anomaly.jpg') # warning: do not specify dpi
fig.savefig('map_sithic_ASO_anomaly.pdf')
#plt.show()
