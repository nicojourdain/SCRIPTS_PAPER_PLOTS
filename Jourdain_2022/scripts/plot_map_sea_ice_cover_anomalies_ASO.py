...
print(ncpA.siconc.isel(time_counter=slice(7,10)).shape)
sic_ASO_p = ( ncpA.siconc.isel(time_counter=slice(7,10)).mean(axis=0) + ncpB.siconc.isel(time_counter=slice(7,10)).mean(axis=0) + ncpC.siconc.isel(time_counter=slice(7,10)).mean(axis=0) ) /3.
sic_ASO_f = ( ncfA.siconc.isel(time_counter=slice(7,10)).mean(axis=0) + ncfB.siconc.isel(time_counter=slice(7,10)).mean(axis=0) + ncfC.siconc.isel(time_counter=slice(7,10)).mean(axis=0) ) /3.
...
tmp_sic_ASO_p = sic_ASO_p * msk_shelf *  ncmsh.e1t.isel(t=0) * ncmsh.e2t.isel(t=0)
tmp_sic_ASO_f = sic_ASO_f * msk_shelf *  ncmsh.e1t.isel(t=0) * ncmsh.e2t.isel(t=0)
tmp_area = msk_shelf *  ncmsh.e1t.isel(t=0) * ncmsh.e2t.isel(t=0)

mean_sic_ASO_p = tmp_sic_ASO_p.sum() / tmp_area.sum()
mean_sic_ASO_f = tmp_sic_ASO_f.sum() / tmp_area.sum()
...
cax0=axs[0].pcolormesh(lon2d, lat2d, sic_ASO_p, vmin=0., vmax=1., rasterized=True, cmap='Blues_r', transform=trans )
#cax0=axs[0].pcolormesh(lon2d, lat2d, sic_ASO_p, rasterized=True, cmap='Blues_r', transform=trans )
...
axs[0].set_title('(c) ASO sea ice concentration (1989-2009)',size=14,fontweight='bold')
...
string=np.array2string(mean_sic_ASO_p.values,precision=3,floatmode='fixed')
...
cax1=axs[1].pcolormesh(lon2d, lat2d, sic_ASO_f, vmin=0., vmax=1., rasterized=True, cmap='Blues_r', transform=trans )
...
axs[1].set_title('(d) ASO sea ice concentration (2080-2100)',size=14,fontweight='bold')
...
# Plot mean value over the Amundsen continental shelf:
string=np.array2string(mean_sic_ASO_f.values,precision=3,floatmode='fixed')
...
#--------------------
#fig.tight_layout()
fig.savefig('map_sic_ASO_anomaly.jpg') # warning: do not specify dpi
fig.savefig('map_sic_ASO_anomaly.pdf')
#plt.show()
