...
ncpA = xr.open_dataset(file_tauU_p_A,decode_cf=False)
ncpB = xr.open_dataset(file_tauU_p_B,decode_cf=False)
ncpC = xr.open_dataset(file_tauU_p_C,decode_cf=False)
ncfA = xr.open_dataset(file_tauU_f_A,decode_cf=False)
ncfB = xr.open_dataset(file_tauU_f_B,decode_cf=False)
ncfC = xr.open_dataset(file_tauU_f_C,decode_cf=False)

print(ncpA.utau.shape)

# Average over A, B, C simulations :
tauU_p = ( ncpA.utau.mean(axis=0) + ncpB.utau.mean(axis=0) + ncpC.utau.mean(axis=0) ) /3.
tauU_f = ( ncfA.utau.mean(axis=0) + ncfB.utau.mean(axis=0) + ncfC.utau.mean(axis=0) ) /3.
...
tmp_tauU_p = tauU_p * msk_shelf *  ncmsh.e1t.isel(t=0) * ncmsh.e2t.isel(t=0)
tmp_tauU_f = tauU_f * msk_shelf *  ncmsh.e1t.isel(t=0) * ncmsh.e2t.isel(t=0)

total_tauU_p = tmp_tauU_p.sum() * 1.e-9 # GN
total_tauU_f = tmp_tauU_f.sum() * 1.e-9 # GN
...
cax0=axs[0].pcolormesh(lon2d, lat2d, tauU_p*1.e3, vmin=-50, vmax=50, rasterized=True, cmap='PuOr_r', transform=trans )
#cax0=axs[0].pcolormesh(lon2d, lat2d, tauU_p*1.e3, rasterized=True, cmap='PuOr_r', transform=trans )
...
axs[0].set_title('(a) Zonal wind & sea-ice stress at ocean surface (1989-2009)',size=14,fontweight='bold')

#colorbar:
cbar0=fig.colorbar(cax0,ax=axs[0],fraction=0.029, pad=0.02, extend='both', ticks=np.arange(-50,60,10))
#cbar0=fig.colorbar(cax0,ax=axs[0],fraction=0.029, pad=0.02, extend='both')
cbar0.ax.set_title('mN/m$^2$',size=12)
...
string=np.array2string(total_tauU_p.values,precision=2,floatmode='fixed')
...
axs[0].text(xmin+0.370*(xmax-xmin), ymin+0.20*(ymax-ymin),'GN',color='black',fontsize=14,fontweight='bold')
...
cax1=axs[1].pcolormesh(lon2d, lat2d, (tauU_f-tauU_p)*1.e3, vmin=-15, vmax=15, rasterized=True, cmap='RdBu_r', transform=trans )
#cax1=axs[1].pcolormesh(lon2d, lat2d, tauU_f-tauU_p, rasterized=True, cmap='RdBu_r', transform=trans )
...
axs[1].set_title('(b) Zonal stress anomaly',size=14,fontweight='bold')

#colorbar:
cbar1=fig.colorbar(cax1,ax=axs[1],fraction=0.029, pad=0.02, extend='both', ticks=np.arange(-15,18,3))
#cbar1=fig.colorbar(cax1,ax=axs[1],fraction=0.029, pad=0.02, extend='both' )
cbar1.ax.set_title('mN/m$^2$',size=12)
...
# Plot mean value over the Amundsen continental shelf:
diff = (total_tauU_f-total_tauU_p).values
...
axs[1].text(xmin+0.365*(xmax-xmin), ymin+0.20*(ymax-ymin),string,color='firebrick',
            fontsize=14,fontweight='bold',ha='right')
axs[1].text(xmin+0.370*(xmax-xmin), ymin+0.20*(ymax-ymin),'GN',color='firebrick',fontsize=14,fontweight='bold')


#--------------------
fig.savefig('map_tauU_anomaly.jpg') # warning: do not specify dpi
fig.savefig('map_tauU_anomaly.pdf')
#plt.show()
