...
npz_TS_file = np.load('TS_profiles_Offshore.npz')
mean_dT = npz_TS_file['mean_Tf'] - npz_TS_file['mean_Tp']

if not os.path.isfile('budget_dT_terms_Offshore.npz'):
  ...
  #for kk in np.arange(mz):
    print('   kk = ',kk)
    msk_shelfA = ncmshA.tmask.isel(t=0,z=kk).where((ncmshA.tmask.isel(t=0,z=kk)==1)&(bathy>1500)&(lon2d>=-135)&(lon2d<=-100.0)&(lat2d<-69.0),0.e0)
    msk_shelfB = ncmshB.tmask.isel(t=0,z=kk).where((ncmshB.tmask.isel(t=0,z=kk)==1)&(bathy>1500)&(lon2d>=-135)&(lon2d<=-100.0)&(lat2d<-69.0),0.e0)
  ...
  np.savez('budget_dT_terms_Offshore.npz',mean_dT_hadv=mean_dT_hadv,\
  ...
  print('  ')
  print('WARNING !!! USING EXISTING budget_dT_terms_Offshore.npz !!!')
  print('  Delete before running this script if you want to regenerate it.')
  print('  ')

  npzfileT = np.load('budget_dT_terms_Offshore.npz')
...
if not os.path.isfile('budget_dS_terms_Offshore.npz'):
  ...
  np.savez('budget_dS_terms_Offshore.npz',mean_dS_hadv=mean_dS_hadv,\
  ...
  print('  ')
  print('WARNING !!! USING EXISTING budget_dS_terms_Offshore.npz !!!')
  print('  Delete before running this script if you want to regenerate it.')
  print('  ')

  npzfileS = np.load('budget_dS_terms_Offshore.npz')
...
col=nico.cbcol(7,withblack=True)
...
#line16, = axs[0].plot(mean_dT_qisf,ZZ,color=col[6])
axs[0].set_xlabel('(c) Conservative Temperature anomaly (degC)',fontsize=16)
axs[0].set_ylabel('Depth (m)',fontsize=14)
axs[0].xaxis.set_ticks_position('top')
axs[0].xaxis.set_label_position('top')
axs[0].invert_yaxis()
axs[0].set_xlim(-10,12.5)
axs[0].set_ylim(1250,0)
axs[0].tick_params(axis='x', labelsize=12)
axs[0].tick_params(axis='y', labelsize=12)
axs[0].grid(True) 
axs[0].legend((line11,line12,line13,line14,line15),\
              ('$\Delta$ T','Horizontal Advection','Vertical Advection','Horizontal mixing','Vert. Mix. + Surf. Fluxes'),fontsize=14)
...
#line26, = axs[1].plot(mean_dS_qisf,ZZ,color=col[6])
axs[1].set_xlabel('(d) Absolute Salinity anomaly (g/kg)',fontsize=16)
...
axs[1].grid(True) 
axs[1].legend((line21,line22,line23,line24,line25),\
              ('$\Delta$ S','Horizontal Advection','Vertical Advection','Horizontal mixing','Vert. Mix. + Surf. Fluxes'),fontsize=14)

fig.savefig('budget_terms_Offshore.jpg')
fig.savefig('budget_terms_Offshore.pdf')
